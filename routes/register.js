...
router.get('/', (req, res) => {
    return res.status(400).json({
        status: 400,
        msg: "Selamat Datang. silahkan registrasi dengan memasukan name, phone, email, password"
    });
})
...
router.post('/', authenticateHeader, async (req, res) => {
    ...
    if (!username || !phone || !email) return res.status(400).json({
        status: 400,
    ...
    if (result1) return res.status(400).json({
        status: 400,
    ...
    if (result2) return res.status(400).json({
        status: 400,
    ...
    if (result3) return res.status(400).json({
        status: 400,
